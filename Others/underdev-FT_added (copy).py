...
class FourierPage(Page):
	def __init__(self, *args, **kwargs):
		...
		self.fourier_analyser = Button(self, text="Fourier Transform", command=self.run_FT_analyser,
			bg=buttonBackground, activebackground=buttonBackground_active)
		self.fourier_analyser.pack(pady=10)

	def run_FT_analyser(self):
...
class MainView(Frame):
    def __init__(self, *args, **kwargs):
        Frame.__init__(self, *args, **kwargs)
        global hp, sstvpage, spectrumpage, fourierpgae
        hp = HomePage()
        sstvpage = SSTVGenPage()
        spectrumpage = FourierPage()
        fourierpgae = fftPage()
        ...
        spectrumpage.place(in_ = self.container, x = 0, y = 0, relwidth = 1, relheight = 1)
